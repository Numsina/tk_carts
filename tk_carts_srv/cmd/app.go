...
import (
	...
	"github.com/Numsina/tk_carts/tk_carts_srv/config"
	"github.com/Numsina/tk_carts/tk_carts_srv/dao"
	"github.com/Numsina/tk_carts/tk_carts_srv/handler"
	"github.com/Numsina/tk_carts/tk_carts_srv/initiallize"
	"github.com/Numsina/tk_carts/tk_carts_srv/initiallize/tracing"
	...
	"github.com/Numsina/tk_carts/tk_carts_srv/tools"
)
...
func (a *App) register() {
	...
	pb.RegisterCartServiceServer(server, web)
...
func (a *App) ioc() *handler.CartHandler {
	err := dao.InitCartsTable(a.db)
	if err != nil {
		panic(err)
	}
	d := dao.New(a.db, a.logger)
	srv := service.NewCartRepository(d, a.logger)
	return handler.NewCartHandler(srv)
}
...
func (a *App) startConsul() {
	...
	srv := &api.AgentServiceRegistration{
		...
		Tags:    []string{"tk_cart_srv", "itgsyang", "cart_srv"},
		Port:    Port,
	...
	if err != nil {
		a.logger.Sugar().Panicf("cart服务注册失败, err: %v", err)
	}

	a.logger.Info("cart服务注册成功....")
}
