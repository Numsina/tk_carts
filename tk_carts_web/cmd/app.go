...
import (
	...
	"github.com/Numsina/tk_carts/tk_carts_web/config"
	"github.com/Numsina/tk_carts/tk_carts_web/handler"
	"github.com/Numsina/tk_carts/tk_carts_web/initiallize"
	prom "github.com/Numsina/tk_carts/tk_carts_web/initiallize/metrics"
	"github.com/Numsina/tk_carts/tk_carts_web/initiallize/tracing"
	"github.com/Numsina/tk_carts/tk_carts_web/logger"
...
func (a *App) Init() {
	...
	initiallize.InitRedis()
	a.logger = initiallize.InitLogger()
	a.jhl = middlewares.NewJWT([]byte(a.conf.JwtInfo.Key))
...
func (a *App) dial() {
	...
	a.conn, err = grpc.NewClient(
		...
		grpc.WithUnaryInterceptor(func(ctx context.Context, method string, req, reply any, cc *grpc.ClientConn, invoker grpc.UnaryInvoker, opts ...grpc.CallOption) error {
			md := metadata.Pairs(
				...
				"client_id", "tk_carts_web",
			)
			ctx = metadata.NewOutgoingContext(ctx, md)
			tracer := otel.Tracer("conn tk_carts_srv")
	...
	a.client = pb.NewCartServiceClient(a.conn)
}
...
func (a *App) registerConsul() {
	...
	if err != nil {
		a.logger.Sugar().Panicf("tk_carts_web consul客户端初始化失败, 失败原因：%s", err)
	}
	...
	svc := &consul.AgentServiceRegistration{
		...
		Tags:    []string{"tk_carts_web", "itgsyang"},
		Check:   chk,
	}
	err = a.consulApi.Agent().ServiceRegister(svc)
	if err != nil {
		a.logger.Sugar().Panicf("tk_carts_web注册微服务失败, 失败原因：%s", err)
	}

	a.logger.Sugar().Info("tk_carts_web注册微服务成功")
}
...
func (a *App) ioc(r *gin.Engine) {
	svc := service.NewCartRepository(a.client)
	carthandler := handler.NewCartHandler(svc)
	carthandler.InitRouter(r)
...
func (a *App) use(r *gin.Engine) {
	...
	r.Use(
		middlewares.NewLoginJWTMiddleWareBuilder(a.jhl).IngorePaths("/metrics", "/health").Build(),
		//metrics.(a.conf.NacosInfo.DataId, a.instanceId, a.conf.ConsuleInfo.Name, "tk_user_web", "统计请求的响应，请求的活跃数， 请求总数").Build(),
		////trace.Trace(),
